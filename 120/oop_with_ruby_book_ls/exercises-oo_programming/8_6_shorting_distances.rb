...
# Make the necessary changes to our code so that the various lengths can be properly sorted and line 62 produces the
# expected output.
#
class Length
  ...
  # I created as specific method for sort as internally it's using the spaceship operator to compare sizes and as I realized
  # that we can sort it via the first letter as we want the specific result and we do not have more details.
  def <=>(other)
    self.unit.to_s[0] <=> other.unit.to_s[0]
  end
