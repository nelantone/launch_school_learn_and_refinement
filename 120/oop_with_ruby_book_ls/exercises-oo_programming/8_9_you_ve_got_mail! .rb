...
module Mailing
  ...
  def send(destination, mail)
    "Sending #{mail} from #{destination.name} to: #{destination.street_address}" # add missing code
...
class EmailService < CommunicationsProvider
  include Mailing # ae to fix issue

  attr_accessor :email_address, :mailbox, :name
...
class PostalService < CommunicationsProvider
  ...
  attr_accessor :street_address, :mailbox, :name # add name missing
